Olá! Meu nome é Lucas Furlani e sou um desenvolvedorapaixonado por tecnologia e aprendizado contínuo. Embora eu ainda não tenha experiência profissional na área, estou entusiasmado e comprometido em iniciar minha carreira e contribuir para projetos de desenvolvimento web.
Habilidades Técnicas
Frontend:
HTML5, CSS3, JavaScript
Frameworks e bibliotecas: React, Angular
Experiência com Bootstrap e outras bibliotecas de design responsivo
Backend:
Linguagens: Node.js, Java
Frameworks: Express.js
Experiência básica com APIs RESTful e GraphQL
Banco de Dados:
Relacionais: MySQL, PostgreSQL
Não relacionais: MongoDB
Outras Ferramentas:
Controle de versão: Git, GitHub
Familiaridade com ferramentas de build e automação: Webpack, Gulp
Conhecimentos básicos em Docker e CI/CD
Formação Acadêmica
Tecnólogo em Análise e Desenvolvimento de Sistemas, FADESC/UNIASSELVI, 2024/1
Desenvolvimento de Sistemas, SENAI São José, 2023/2
Projetos Pessoais



